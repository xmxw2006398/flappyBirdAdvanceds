...
static const uint32_t enemyCategory = 1 << 4;
...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        _zombies = [SKNode node];
        [_moving addChild:_zombies];
        ...
        _enemyTexture = [SKTexture textureWithImageNamed:@"zombie"];
        _enemyTexture.filteringMode = SKTextureFilteringNearest;
        SKAction *actionMove =
        [SKAction moveByX:-self.size.width + _enemy.size.width y:0 duration:1.0];
        SKAction *actionRemove = [SKAction removeFromParent];
        _moveEnemy = [SKAction sequence:@[actionMove, actionRemove]];
        
        SKAction* spawn1 = [SKAction performSelector:@selector(spawnEnemy) onTarget:self];
        SKAction* delay1 = [SKAction waitForDuration:2.0];
        SKAction* spawnThenDelay1 = [SKAction sequence:@[spawn1, delay1]];
        SKAction* spawnThenDelayForever1 = [SKAction repeatActionForever:spawnThenDelay1];
...
- (void)spawnEnemy
{
    SKSpriteNode *enemy = [SKSpriteNode spriteNodeWithTexture:_enemyTexture];
    [enemy setScale:1];
    CGPoint enemyScenePos = CGPointMake(
                                        self.size.width + _enemy.size.width/2,
                                        floorf(((double)arc4random() / ARC4RANDOM_MAX) *
                                               (self.size.height - _enemy.size.height/2) + _enemy.size.height/2));
    enemy.position = [self convertPoint:enemyScenePos toNode:_moving];
    enemy.physicsBody = [SKPhysicsBody bodyWithRectangleOfSize:enemy.size];
     enemy.physicsBody.dynamic = YES;
     enemy.physicsBody.allowsRotation = NO;
     enemy.physicsBody.categoryBitMask = enemyCategory;
     enemy.physicsBody.collisionBitMask = birdCategory;
     enemy.physicsBody.contactTestBitMask = birdCategory;
    [enemy runAction:_moveEnemy];
    [_zombies addChild:enemy];
}
...
- (void)didBeginContact:(SKPhysicsContact *)contact {
    if( _moving.speed > 0 ) {
        if( ( contact.bodyA.categoryBitMask & scoreCategory ) == scoreCategory || ( contact.bodyB.categoryBitMask & scoreCategory ) == scoreCategory ) {
            ...
            _scoreLabelNode.text = [NSString stringWithFormat:@"Gameover your score is:%d", _score];
